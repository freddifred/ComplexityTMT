...
function showTab(name, prev) {
	...
	if (tmp[name] && player.tab === name && isPlainObject(tmp[name].tabFormat)) {
		if (name == "atomic" && hasMilestone("Progression", 1)) {player.subtabs[name].mainTabs = Object.keys(layers[name].tabFormat)[1]}
		else {player.subtabs[name].mainTabs = Object.keys(layers[name].tabFormat)[0]}
